...
class AttendanceOverviewController extends Controller
{
    ...
    private function dailyAttendanceSummary(Carbon $date, Collection $activeEmployeeIds): array
    {
        ...
        $onTimeCount = (clone $attendanceTodayQuery)->whereNotNull('clock_in')->whereRaw('LOWER(COALESCE(status, "")) = ?', ['masuk'])->distinct()->count('employee_id');
        $lateCount = (clone $attendanceTodayQuery)->whereNotNull('clock_in')->where(function ($query): void {
            $query->whereRaw('LOWER(COALESCE(status, "")) = ?', ['terlambat'])->orWhere('late_minutes', '>', 0);
    ...
    private function dailyAttendanceLists(Carbon $date, Collection $activeEmployeeIds): array
    {
        ...
        $dailyEarlyBirds = (clone $attendanceTodayQuery)->whereRaw('LOWER(COALESCE(status, "")) = ?', ['masuk'])->orderBy('clock_in')->limit(5)->get(['id', 'employee_id', 'date', 'clock_in', 'status', 'late_minutes'])->values()->map(fn (Attendance $attendance, int $index): array => $this->presentAttendancePerson($attendance, $index + 1));
        $dailyRunningLate = (clone $attendanceTodayQuery)->where(function ($query): void {
            $query->whereRaw('LOWER(COALESCE(status, "")) = ?', ['terlambat'])->orWhere('late_minutes', '>', 0);
        })->orderBy('clock_in')->limit(5)->get(['id', 'employee_id', 'date', 'clock_in', 'status', 'late_minutes'])->values()->map(fn (Attendance $attendance, int $index): array => $this->presentAttendancePerson($attendance, $index + 1));
    ...
    private function weeklyAttendanceCharts(Carbon $weekStart, Collection $activeEmployeeIds): array
    {
        ...
        $onTimeByDay = $attendanceRows->filter(fn (Attendance $attendance): bool => strtolower((string) $attendance->status) === 'masuk')->groupBy(fn (Attendance $attendance): string => $this->dateKey($attendance->date))->map(fn (Collection $rows): int => $rows->pluck('employee_id')->unique()->count());
        $lateByDay = $attendanceRows->filter(fn (Attendance $attendance): bool => strtolower((string) $attendance->status) === 'terlambat' || (int) $attendance->late_minutes > 0)->groupBy(fn (Attendance $attendance): string => $this->dateKey($attendance->date))->map(fn (Collection $rows): int => $rows->pluck('employee_id')->unique()->count());
