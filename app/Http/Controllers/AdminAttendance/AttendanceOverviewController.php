...
class AttendanceOverviewController extends Controller
{
    ...
    private function dailyAttendanceLists(Carbon $date, Collection $activeEmployeeIds): array
    {
        ...
        $dailyRunningLate = (clone $attendanceTodayQuery)->where('late_minutes', '>', 0)->orderBy('clock_in')->limit(5)->get(['id', 'employee_id', 'date', 'clock_in', 'status', 'late_minutes'])->values()->map(fn (Attendance $attendance, int $index): array => $this->presentAttendancePerson($attendance, $index + 1));
