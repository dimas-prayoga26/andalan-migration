...
class PicAttendanceLeaveController extends Controller
{
    ...
    public function detail(Request $request, string $uid): View
    {
        ...
        return view('pic_attendance.leave.detail', [
    ...
    public function updateSupervisorReview(Request $request, string $uid): RedirectResponse
    {
        ...
        DB::transaction(function () use ($activeEmployeeIds, $authenticatedUser, $now, $targetStatus, $uid): void {
            ...
            $supervisorReview = LeaveRequestHistory::query()
                ...
                ->lockForUpdate()
                ->first();
            $hasCompletedSupervisorReview = LeaveRequestHistory::query()
                ->where('leave_request_id', $leaveRequest->id)
                ->where('event_type', 'supervisor_review')
                ->whereIn('to_status', ['approved', 'rejected'])
                ->exists();
            if ($hasCompletedSupervisorReview) {
                throw ValidationException::withMessages([
                    'status' => 'Leave request sudah melalui Supervisor Review.',
                ]);
            }

            $actorUserId = $authenticatedUser instanceof User ? (string) $authenticatedUser->id : null;
            if ($supervisorReview instanceof LeaveRequestHistory) {
        ...
        return redirect()
            ->route('pic-attendance.leave.detail', ['uid' => $uid])
            ->with('success', 'Supervisor Review berhasil '.($targetStatus === 'approved' ? 'disetujui.' : 'ditolak.'));
    }
    ...
    private function leaveOverviewStatsFor(Request $request): array
    {
        ...
        return [
            'pending' => $this->countPendingSupervisorReviewLeaveRequests($activeEmployeeIds),
            'rejected' => (int) ($statusCounts['rejected'] ?? 0) + (int) ($statusCounts['refused'] ?? 0),
            'approved' => (int) $this->supervisorApprovedLeaveRequestQuery($activeEmployeeIds)->count(),
    ...
    private function currentCompanyIdFor(User $user): ?string
    {
        ...
        $supervisorEmployeeId = $user->employee?->id;
        $this->supervisorEmployeeId = is_string($supervisorEmployeeId) && trim($supervisorEmployeeId) !== ''
    ...
    private function countPendingSupervisorReviewLeaveRequests(Collection $activeEmployeeIds): int
    {
        return (int) $this->applyPendingSupervisorReviewFilter(
            $this->baseLeaveRequestQuery($activeEmployeeIds)
    ...
    private function leaveRequestsDatatable(Request $request, string $status): JsonResponse
    {
        ...
        if (strtolower($status) === 'pending') {
            $query = $this->applyPendingSupervisorReviewFilter(
                $query->whereRaw('LOWER(COALESCE(status, "")) = ?', ['pending'])
            );
    ...
    private function applyPendingSupervisorReviewFilter(Builder $query): Builder
    {
        return $query->whereDoesntHave('histories', function (Builder $query): void {
            $query
                ->where('event_type', 'supervisor_review')
                ->whereIn('to_status', ['approved', 'rejected']);
        });
    ...
    private function leaveApprovalFor(LeaveRequest $leaveRequest): array
    {
        ...
        $supervisorReview = $leaveRequest->histories
            ->filter(static fn (LeaveRequestHistory $history): bool => strtolower(trim((string) $history->event_type)) === 'supervisor_review')
            ->sortByDesc('happened_at')
            ->first();
        $supervisorReviewStatus = $supervisorReview instanceof LeaveRequestHistory
            ? strtolower(trim((string) $supervisorReview->to_status))
            : 'pending';
        $isPending = $requestStatus === 'pending' && $supervisorReviewStatus === 'pending';
        ...
        $attachmentUrl = $isSickLeave && $attachmentPath !== ''
            ...
            : null;
        $status = $requestStatus === 'pending' ? $supervisorReviewStatus : $requestStatus;
