# Driver Tanpa Pengurangan Rest Attendance

Tanggal: 2026-08-11 09:09 WIB

## Ringkasan

- Mengubah perhitungan jam kerja attendance agar employee dengan posisi `Driver` tidak terkena pengurangan rest 1 jam.
...
- Jika employee memiliki posisi aktif `Driver`, durasi kerja dihitung dari selisih penuh `clock_in` sampai `clock_out`.
- Jika employee bukan `Driver`, durasi kerja tetap memakai aturan lama: durasi minimal 6 jam dikurangi rest 1 jam.
- Contoh:
  - `Driver`: `08:00 - 17:00 = 9 hours`.
  - Non-driver: `08:00 - 17:00 = 8 hours`.

## Cara Sistem Membaca Posisi Driver

- Helper `Employee::hasPositionName('Driver')` membaca posisi dari:
...
- Saat ini nama posisi `Driver` masih dibaca langsung dari nama position, sehingga jika nama position diubah, logic juga perlu disesuaikan.

## Area yang Disesuaikan

- Clock-out attendance menghitung `work_hours` tanpa pengurangan rest untuk Driver.
- Pengajuan attendance exception yang menghitung ulang jam kerja juga mengikuti pengecualian Driver.
- Recap Admin Attendance mengikuti aturan Driver saat menghitung label jam kerja dan total bulanan.
- Recap PIC Attendance mengikuti aturan yang sama.
- Staff Attendance Report mengikuti aturan Driver pada label jam kerja.
- Attendance profile composer mengikuti aturan Driver untuk hitungan weekly worked hours, termasuk batas maksimum harian Driver menjadi 540 menit agar tidak terpotong ke 480 menit.
