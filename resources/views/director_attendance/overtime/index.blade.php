...
@section('css')
    ...
    <style>
        .director-overtime-table-footer.dataTables_wrapper {
            display: flex;
            align-items: center;
        ...
        .director-overtime-review-table {
            height: 286px;
            overflow-x: auto;
            overflow-y: hidden;
        ...
        .director-overtime-review-table::-webkit-scrollbar {
            height: 7px;
        }

        .director-overtime-review-table::-webkit-scrollbar-track {
            background: #f1f3f7;
            border-radius: 999px;
        }

        .director-overtime-review-table::-webkit-scrollbar-thumb {
            background: #c6ccd6;
            border-radius: 999px;
        }

        .director-overtime-review-table::-webkit-scrollbar-thumb:hover {
            background: #aab2c0;
        }
        ...
        .director-overtime-table-footer.dataTables_wrapper .dataTables_info,
        .director-overtime-table-footer.dataTables_wrapper .dataTables_paginate {
            float: none;
            margin-bottom: 0;
            padding: 0;
        }

        .director-overtime-table-footer .paginate_button.disabled {
            pointer-events: none;
            opacity: .35;
        }
...
@php
    $directorOvertimeTablePageSize = 5;
    ...
    $pendingTablePageCount = max(1, (int) ceil($pendingTableRows->count() / $directorOvertimeTablePageSize));
    $approvedTablePageCount = max(1, (int) ceil($approvedTableRows->count() / $directorOvertimeTablePageSize));
@endphp
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body table-card-body px-0 pt-0 pb-0" data-director-overtime-table data-director-overtime-page-size="{{ $directorOvertimeTablePageSize }}">
                        <div class="table-responsive director-overtime-review-table overtime-review-table">
                            <table class="table table-sm table-sm-responsive text-nowrap overtime-pending-table" id="tableLicenseUsage">
                                ...
                                <tbody>
                                    @forelse ($pendingTableRows as $row)
                                        <tr @class(['js-director-overtime-table-row', 'd-none' => $loop->iteration > $directorOvertimeTablePageSize])>
                        ...
                        <div class="director-overtime-table-footer dataTables_wrapper no-footer">
                            <div class="dataTables_info js-director-overtime-page-summary">
                                Showing {{ $pendingTableRows->isEmpty() ? 0 : 1 }} to {{ min($directorOvertimeTablePageSize, $pendingTableRows->count()) }} of {{ $pendingTableRows->count() }} entries
                            </div>
                            <div class="dataTables_paginate paging_simple_numbers js-director-overtime-pagination">
                                <a href="javascript:void(0)" class="paginate_button previous disabled js-director-overtime-page-button" data-director-overtime-page-action="previous" data-director-overtime-page-item="previous" aria-label="Previous page"><i class="fa-solid fa-angle-left"></i></a>
                                <span>
                                    @for ($page = 1; $page <= $pendingTablePageCount; $page++)
                                        <a href="javascript:void(0)" class="paginate_button {{ $page === 1 ? 'current' : '' }} js-director-overtime-page-button" data-director-overtime-page="{{ $page }}" data-director-overtime-page-item="{{ $page }}" @if ($page === 1) aria-current="page" @endif>{{ $page }}</a>
                                    @endfor
                                </span>
                                <a href="javascript:void(0)" class="paginate_button next {{ $pendingTablePageCount <= 1 ? 'disabled' : '' }} js-director-overtime-page-button" data-director-overtime-page-action="next" data-director-overtime-page-item="next" aria-label="Next page"><i class="fa-solid fa-angle-right"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body table-card-body px-0 pt-0 pb-0" data-director-overtime-table data-director-overtime-page-size="{{ $directorOvertimeTablePageSize }}">
                        ...
                        <div class="director-overtime-table-footer dataTables_wrapper no-footer">
                            <div class="dataTables_info js-director-overtime-page-summary">
                                Showing {{ $approvedTableRows->isEmpty() ? 0 : 1 }} to {{ min($directorOvertimeTablePageSize, $approvedTableRows->count()) }} of {{ $approvedTableRows->count() }} entries
                            </div>
                            <div class="dataTables_paginate paging_simple_numbers js-director-overtime-pagination">
                                <a href="javascript:void(0)" class="paginate_button previous disabled js-director-overtime-page-button" data-director-overtime-page-action="previous" data-director-overtime-page-item="previous" aria-label="Previous page"><i class="fa-solid fa-angle-left"></i></a>
                                <span>
                                ...
                                <a href="javascript:void(0)" class="paginate_button next {{ $approvedTablePageCount <= 1 ? 'disabled' : '' }} js-director-overtime-page-button" data-director-overtime-page-action="next" data-director-overtime-page-item="next" aria-label="Next page"><i class="fa-solid fa-angle-right"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="d-flex justify-content-end mb-3 director-overtime-card-filter">
                    <form class="clearfix d-flex align-items-center" method="GET" action="{{ url()->current() }}">
                        <input type="hidden" name="pending_month" value="{{ $selectedPendingMonth }}">
                        <input type="hidden" name="pending_year" value="{{ $selectedPendingYear }}">
                        <input type="hidden" name="approved_month" value="{{ $selectedApprovedMonth }}">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="clearfix d-flex">
                                <div class="avatar avatar-sm rounded me-3">
                                    <span class="avatar-title rounded bg-primary-light text-primary fw-semibold">
                                        {{ \Illuminate\Support\Str::substr($overtimeCard['employee_name'] ?? 'S', 0, 1) }}
                                    </span>
                                </div>
                                <div class="clearfix">
                            ...
                            <div class="row py-1">
                                ...
                                <div class="col-12">
                                    @foreach (($overtimeCard['time_lines'] ?? []) as $timeLine)
                                        <span @class(['text-decoration-line-through' => $timeLine['strike'] ?? false])>{{ $timeLine['label'] ?? '-' }}</span><br>
                                    @endforeach
                                </div>
                            </div>
                            <div class="row py-1">
                        ...
                        <div class="card-footer d-flex justify-content-between flex-wrap">
                            <p class="mb-0 fw-semibold">
                                <span class="{{ ($overtimeCard['current_log']['status'] ?? '') === 'Approved' ? 'text-success' : 'text-warning' }}">
                                    {{ $overtimeCard['current_log']['title'] }}
                                </span>
                            </p>
                            <span class="badge badge-sm {{ $overtimeCard['current_log']['badge_class'] ?? 'badge-warning' }} light fw-semibold">
                                {{ $overtimeCard['current_log']['status'] ?? 'Pending' }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body text-center text-muted">
                            No director approval overtime cards available for this period.
                        </div>
                    </div>
                </div>
            @endforelse
...
@section('script')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function directorOvertimeTablePageSize($tableContainer) {
                return parseInt($tableContainer.attr('data-director-overtime-page-size'), 10) || 5;
            }

            function updateDirectorOvertimeTablePagination($tableContainer, currentPage, totalPages, totalRows) {
                var pageSize = directorOvertimeTablePageSize($tableContainer);
                var start = totalRows === 0 ? 0 : ((currentPage - 1) * pageSize) + 1;
                var end = Math.min(currentPage * pageSize, totalRows);

                $tableContainer.find('.js-director-overtime-page-summary').text('Showing ' + start + ' to ' + end + ' of ' + totalRows + ' entries');
                $tableContainer.find('[data-director-overtime-page-item]').removeClass('current disabled');
                $tableContainer.find('[data-director-overtime-page]').removeAttr('aria-current');
                $tableContainer.find('[data-director-overtime-page-item="' + currentPage + '"]')
                    .addClass('current')
                    .attr('aria-current', 'page');

                if (currentPage <= 1) {
                    $tableContainer.find('[data-director-overtime-page-item="previous"]').addClass('disabled');
                }

                if (currentPage >= totalPages) {
                    $tableContainer.find('[data-director-overtime-page-item="next"]').addClass('disabled');
                }
            }

            function showDirectorOvertimeTablePage(tableContainer, pageNumber) {
                var $tableContainer = jQuery(tableContainer);
                var $rows = $tableContainer.find('.js-director-overtime-table-row');
                var pageSize = directorOvertimeTablePageSize($tableContainer);
                ...
                $tableContainer.attr('data-director-overtime-current-page', currentPage);
                updateDirectorOvertimeTablePagination($tableContainer, currentPage, totalPages, totalRows);
            }

            function initDirectorOvertimeTablePagination() {
                jQuery('[data-director-overtime-table]').each(function () {
                    showDirectorOvertimeTablePage(this, jQuery(this).attr('data-director-overtime-current-page') || 1);
                });
            }

            jQuery(document).on('click', '.js-director-overtime-page-button', function () {
                ...
                var $tableContainer = $button.closest('[data-director-overtime-table]');
                var currentPage = parseInt($tableContainer.attr('data-director-overtime-current-page'), 10) || 1;
                var pageAction = $button.attr('data-director-overtime-page-action');
                var targetPage = parseInt($button.attr('data-director-overtime-page'), 10) || currentPage;
                ...
                showDirectorOvertimeTablePage($tableContainer, targetPage);
            });

            initDirectorOvertimeTablePagination();
        });
    </script>
@endsection
